...
/**
 * CU08 — Reportes del negocio (módulo reporte). Solo lectura.
 * Devuelve arrays de datos; la presentación (JSON o PDF) la arma el controlador.
 */
class ReporteService
{
    /** Registra en auditoría que se generó un reporte. */
    private function registrarAuditoria(int $actorId, string $tipo, array $params): void
    {
        Reporte::create([
            'propietario_id' => $actorId,
    ...
    public function ventas(?string $ini, ?string $fin, int $actorId, array $params): array
    {
        $q = Venta::query();
        if ($ini) {
        ...
        $this->registrarAuditoria($actorId, 'REPVEN', $params);
    ...
    public function encomiendas(?string $ini, ?string $fin, int $actorId, array $params): array
    {
        $q = Encomienda::query();
        if ($ini) {
        ...
        $this->registrarAuditoria($actorId, 'REPENC', $params);
    ...
    public function inventario(int $actorId, array $params): array
    {
        ...
        $this->registrarAuditoria($actorId, 'REPINV', $params);
    ...
    public function cotizaciones(?string $ini, ?string $fin, int $actorId, array $params): array
    {
        $q = Cotizacion::query();
        if ($ini) {
        ...
        $this->registrarAuditoria($actorId, 'REPCOT', $params);
    ...
    public function facturas(?string $ini, ?string $fin, int $actorId, array $params): array
    {
        $q = Factura::query();
        if ($ini) {
            $q->whereBetween('fecha_emision', [$ini . ' 00:00:00', $fin . ' 23:59:59']);
        ...
        $this->registrarAuditoria($actorId, 'REPFAC', $params);
